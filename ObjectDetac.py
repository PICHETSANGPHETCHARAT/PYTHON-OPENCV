...
cred = credentials.Certificate(
    './pet007-1c21d-firebase-adminsdk-b6v3j-81c70cd241.json')
...
while True:
    ...
    if ret:
        ...
        if CLASSES[class_index] == "CAT":
            ref.update({
                'Status': 'แมวกำลังเดิน'
            })
        else:
            ref.update({
                'Status': 'แมวกำลังอยู่กับที่'
            })
        if CLASSES[class_index] == "DOG":
            ref.update({
                'Status': 'หมากำลังเดิน'
            })
        else:
            ref.update({
                'Status': 'หมากำลังอยู่กับที่'
            })
