...
while cap.isOpened():
    ...
    img_resized = cv2.resize(imgRGB, size, fx=scale_value,
    ...
    cv2.putText(img, class_name, (75, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
    cv2.putText(img, str(float("{:.2f}".format(confidence_score*100))) +
                "%", (75, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)

    if class_name == "Pet Sleep":
        ref.update({
            'Status': 'สัตว์เลี้ยงกำลังนอน'
        })
    if class_name == "Pet Walk":
        ref.update({
            'Status': 'สัตว์เลี้ยงกำลังเดิน'
        })
    if class_name == "Pet Eat":
        ref.update({
            'Status': 'สัตว์เลี้ยงกำลังกินอาหาร'
        })    
